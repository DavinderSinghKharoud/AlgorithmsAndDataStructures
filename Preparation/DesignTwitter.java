...
public class DesignTwitter {
    ...
    public void postTweet(int userId, int tweetId) {
        Tweet curr = new Tweet(timeStamp++, tweetId);
        if (!users.containsKey(userId)) {
            users.put(userId, curr);
            //Also add user as itself a follower
            if (!followed.containsKey(userId)) {
                followed.put(userId, new HashSet<>());
            }
            followed.get(userId).add(userId);
        } else {
            curr.next = users.get(userId);
            users.put(userId, curr);
        }
    }
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        for (int followedUsers : followed.get(userId)) {
            if (users.containsKey(followedUsers)
                    && users.get(followedUsers) != null) {
                System.out.println(followedUsers);
                pq.add(users.get(followedUsers)); //Add tweets
            }
        }

        while (pq.size() != 0 && res.size() < max) {
            Tweet curr = pq.poll();
            res.add(curr.id);
            if (curr.next != null) {
                pq.add(curr.next);
