...
public class ArrangingASheep implements Runnable {
   ...
   void solve() throws IOException {
      ...
      while (t-- > 0) {
         ...
         long[] prefix = lArr(n), suffix = lArr(n);
         preCompute(arr, prefix, suffix);
         long min = Long.MAX_VALUE;
         for (int i = 0; i < n; i++) {
            min = min(prefix[i] + ((i + 1 < n) ? suffix[i + 1] : 0), min);
            min = min(suffix[i] + ((i - 1 >= 0) ? prefix[i - 1] : 0), min);
         }
         println(min);
      }
   }

   void preCompute(char[] arr, long[] prefix, long[] suffix) {
      long steps = 0;
      int base = 0;
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new ArrangingASheep(), "1").start();
   }
