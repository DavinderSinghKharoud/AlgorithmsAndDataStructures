...
public class StrangeSubsequences implements Runnable {
   ...
   void solve() throws IOException {
      ...
      while (t-- > 0) {
         ...
         boolean take = (min != dmax);
         for (int j = 1; j < n && arr[j] <= 0; j++) {
            take &= (arr[j] - arr[j - 1]) >= min;
         }
         if (take)
            ans++;
         println(ans);
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new StrangeSubsequences(), "1", 1 << 26).start();
   }
