...
public class Permutations implements Runnable {
    ...
    void solve() throws IOException {
        int t = ri();
        ArrayDeque<Integer>[] arr = new ArrayDeque[limit];

        for (int i = 0; i < t; i++) {
            int curr = ri();
            if (arr[curr] == null) {
                arr[curr] = new ArrayDeque<>();
            }
            arr[curr].add(i);
        }
        ...
        if (arr[1] == null) {
            ...
            while (!arr[1].isEmpty()) {
                ans[arr[1].poll()] = count;

                for (int i = 2; i < arr.length; i++) {
                    if (arr[i] == null || arr[i].isEmpty())
                        break;
                    ans[arr[i].poll()] = count;
                }
                count++;
                total++;
            }
    ...
    public static void main(String[] args) throws IOException {
        new Thread(null, new Permutations(), "1").start();
    }
