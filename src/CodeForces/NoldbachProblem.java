...
public class NoldbachProblem implements Runnable {
   ...
   void solve() throws IOException {
      ...
      for (int i = 1; i < primes.size(); i++) {
         int next = primes.get(i) + primes.get(i - 1) + 1;
         if (next <= n && set.contains(next))
            count++;
      }
      println((count >= k) ? "YES" : "NO");
   }
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new NoldbachProblem(), "1").start();
   }
