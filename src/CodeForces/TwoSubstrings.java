...
public class TwoSubstrings implements Runnable {

   void solve() throws IOException {
      String s = rs();

      for (int i = 0; i < s.length(); i++) {
         if (s.charAt(i) == 'A' && i + 1 < s.length() && s.charAt(i + 1) == 'B') {

            // Find the occurrences of BA
            for (int j = 0; j < s.length(); j++) {
               char curr = s.charAt(j);
               if (curr == 'B' && j + 1 < s.length() && s.charAt(j + 1) == 'A') {
      ...
      for (int i = 0; i < s.length(); i++) {
         ...
         if (s.charAt(i) == 'B' && i + 1 < s.length() && s.charAt(i + 1) == 'A') {

            // Find the occurrences of BA
            for (int j = 0; j < s.length(); j++) {
               char curr = s.charAt(j);
               if (curr == 'A' && j + 1 < s.length() && s.charAt(j + 1) == 'B') {
                  // Check if it is overlapping
                  if (i != j && i + 1 != j && i != j + 1 && i + 1 != j + 1) {
                     println("YES");
                     return;
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new TwoSubstrings(), "1").start();
   }
