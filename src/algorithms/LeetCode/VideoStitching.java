...
public class VideoStitching {
    ...
    static int videoStitching2(int[][] clips, int T) {
        ...
        int max = 0, res = 0, currMax = 0;

        for (int index = 0; index < T; index++) {
            max = Math.max( max, maxEnd[index]);
            if( max == index) { //We cannot go any further as there is a gap
                return -1;
            }
