...
public class SmallerStrings implements Runnable {
   ...
   void solve() {
      ...
      for (int tt = 1; tt <= t; tt++) {
         ...
         for (int i = 0; i < (len + 1) / 2; i++) {
            // Find all the smaller strings and we can easily make them palindrome by
            // appending reverse
            ans = (ans * k + (s.charAt(i) - 'a')) % mod;
         }

         // We need to check if first part of string is lexicographically smaller or not
         boolean good = false;
         for (int i = 0; i < len / 2; i++) {
            char a = s.charAt(len / 2 - 1 - i), b = s.charAt((len + 1) / 2 + i);
            if (a == b)
               continue;
            else if (a < b) {
               good = true;
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new SmallerStrings(), "1").start();
   }
