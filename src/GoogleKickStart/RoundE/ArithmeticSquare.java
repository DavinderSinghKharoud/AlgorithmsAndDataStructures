...
public class ArithmeticSquare implements Runnable {
   ...
   void solve() {
      ...
      for (int tt = 1; tt <= t; tt++) {
         ...
         int res = 0;
         // Count for all four sides
         if (isArith(grid[0][0], grid[0][1], grid[0][2])) {
            res++;
         }
         if (isArith(grid[0][0], grid[1][0], grid[2][0])) {
            res++;
         }
         if (isArith(grid[2][0], grid[2][1], grid[2][2])) {
            res++;
         }
         if (isArith(grid[0][2], grid[1][2], grid[2][2])) {
            res++;
         }

         Map<Long, Integer> map = new HashMap<Long, Integer>();
         // Get the middles if possible
         // row and column
         long sum = grid[1][0] + grid[1][2];
         if (sum % 2 == 0) {
         ...
         println(res + ((max != dmin) ? max : 0));
      }
   }

   boolean isArith(long a, long b, long c) {
      return (b - a) == (c - b);
   }

   /************************************************************************************************************************************************/
   public static void main(String[] args) throws IOException {
      new Thread(null, new ArithmeticSquare(), "1").start();
   }
