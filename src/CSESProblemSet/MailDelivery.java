 ...
 * ,
 * n
 * , and the post office is located at crossing 1
 * 1
 * .
 *
 * After that, there are m
 * m
 *  lines describing the streets. Each line has two integers a
 * a
 *  and b
 * b
...
public class MailDelivery {
    ...
    static void solve() throws IOException {
        ...
        int edgeCount = m;
        while (m-- > 0) {
            int a = fastReader.intNext(), b = fastReader.intNext();
            --a;
            --b;
            arr.get(a).add(b);
            arr.get(b).add(a);
        ...
        deque.add(0);
        while ( !deque.isEmpty() ){
            while ( !arr.get(deque.peek()).isEmpty() ){
                int u = deque.peek(), v = arr.get(u).poll();
                arr.get(v).remove(u);
                deque.add(v);
            }
            res.add( deque.pop());
        ...
        if ((res.size() ^ (edgeCount + 1)) != 0) {
            print("IMPOSSIBLE");
            return;
        }

        for (Integer r : res) {
            sbr.append(r + 1).append(" ");
        }
        print(sbr);
    }
	...
	static void dfs( int node, List<Integer> res, boolean[] vis, int[] eu, int[] ev){
		while( !arr.get(node).isEmpty() ){
			int id = arr.get(node).pollLast();
			if( vis[id]) continue;
			vis[id] = true;
			dfs( (ev[id] ^ eu[id] ^ node), res, vis, eu, ev);
			res.add(node);
		}
	}
    /************************************************************************************************************************************************/
