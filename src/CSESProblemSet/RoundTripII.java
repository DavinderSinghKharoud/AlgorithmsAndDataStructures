...
public class RoundTripII {
    ...
    static void solve() throws IOException {
		...
		for(int node = 0; node < n; node++ ){
			if( !visited[node]){
				dfs(node, visited, parent, isActive, arr);
			}
			if(isPossible) break;
		}
		
		if(!isPossible) print("IMPOSSIBLE");
	

    }

	static void dfs( int node, boolean[] visited, int[] parent, boolean[] isActive ,List<List<Integer>> arr ){
		visited[node] = true;
		isActive[node] = true;
		...
		for(int adj: arr.get(node) ){
			...
			if( isActive[adj]){
				...
				parent[adj] = node;
				dfs( adj, visited, parent, isActive, arr);
			}
