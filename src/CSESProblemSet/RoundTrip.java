...
public class RoundTrip {
    ...
    static void solve() throws IOException {
		...
		for(int node = 0; node < n; node++ ){
			if( visited[node] == null){
				if( !dfs( lst, visited, new Node(node,0, null) )) break;
			}
		}
    ...
    static boolean dfs( List<List<Integer>> lst, Node[] visited, Node node){
			
			visited[node.value] = node;
			
			for(int adj: lst.get(node.value) ){
				if( visited[adj] == null){
					
					if( !dfs( lst, visited, new Node(adj, node.distance + 1, node) ) ){
						return false;
					}
				}else{
					int diff = Math.abs( node.distance - visited[adj].distance );
					if( diff >= 2 ){
						isPossible = true;
						
						List<Integer> res = new ArrayList<>();
						res.add(adj);
						while(node.value != adj ){
							res.add(node.value);
							node = node.parent;
						}
						res.add(adj);
