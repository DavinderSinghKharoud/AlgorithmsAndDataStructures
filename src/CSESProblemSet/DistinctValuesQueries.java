...
public class DistinctValuesQueries {
	...
	static int n;
	static int[] arr, bit;
	static List<List<int[]>> queries = new ArrayList<>();
    static void solve() throws IOException {
		...
		for(int i = 1; i <= n; i++ ){
			...
			if( map.containsKey(arr[i]) ){
				update(map.get(arr[i]), -1);
			}
			
			map.put(arr[i], i);
			//Answer all the queries which have i as right side
			
			for( int[] query: queries.get(i)){
				ans[query[1]] = query(i) - query(query[0] - 1);
			}
