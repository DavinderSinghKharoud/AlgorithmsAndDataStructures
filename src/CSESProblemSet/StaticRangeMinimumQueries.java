...
public class StaticRangeMinimumQueries {
    ...
    static void solve() throws IOException {
        ...
        int n = fastReader.intNext(), q = fastReader.intNext();

        int log =  log2(n);
        int[][] dp = new int[n][log + 1];

        for (int i = 0; i < n; i++) {
            dp[i][0] = fastReader.intNext();
        }
        ...
        while (q-- > 0) {
            ...
            int min = min(dp[a][k], dp[a + len - (int) Math.pow(2, k)][k]);
            if (dp[a][k] <= dp[b - (1 << k) + 1][k]) {
                println(dp[a][k]);
            } else {
                println(dp[b - (1 << k) + 1][k]);
            }
            //println(min(dp[a][k], dp[b - (1 << k) + 1][k]));
        }
