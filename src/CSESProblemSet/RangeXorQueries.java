 ...
 * q
 *  queries of the form: what is the xor sum of values in range [a,b]
 * [
 * a
 * ,
 * b
...
public class RangeXorQueries {
    ...
    static void solve() throws IOException {
		...
		while( m--> 0 ){
			...
			println( findXor(0, n - 1, a, b, 0) );
		}
    }
    
    static int findXor(int l, int h, int qa, int qb, int pos ){
		if( qa <= l && qb >= h ){
			return dp[pos];
		}
		if( qa > h || qb < l ){
		...
		int mid = l + (h - l)/2;
		return findXor(l, mid, qa, qb, 2 * pos + 1) ^ findXor( mid + 1, h, qa, qb, 2 * pos + 2);
	}
