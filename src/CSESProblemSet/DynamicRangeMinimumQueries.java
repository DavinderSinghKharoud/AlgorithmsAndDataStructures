...
public class DynamicRangeMinimumQueries {
    ...
    static void solve() throws IOException {
        ...
        while (m-- > 0) {
            int t = fastReader.intNext(), a = fastReader.intNext() - 1, b = fastReader.intNext();
            if (t == 1) {
                update(0, n - 1, 0, a, b);
            } else {
                println(findMin(0, n - 1, a, b - 1, 0));
            }
        }
    }

    static int update(int l, int h, int pos, int target, int value) {
		
		if( target < l || target > h ) return dp[pos];
		if( l == h ){
			dp[pos] = value;
			return value;
		}
		...
		dp[pos] = min( update(l , mid, 2 * pos + 1,target,  value), update(mid + 1, h, 2 * pos + 2, target, value));
		return dp[pos];
    }

    static int findMin(int l, int h, int a, int b, int pos) {
        if ( a <= l && b >= h) {
            //Complete Overlapp
            return dp[pos];
        }
        ...
        return min(findMin(l, mid, a, b, 2 * pos + 1), findMin(mid + 1, h, a, b, 2 * pos + 2));
    }
    ...
    static void constructTree(int[] arr, int l, int h, int pos) {
        ...
        dp[pos] = min(dp[2 * pos + 1], dp[2 * pos + 2]);
    }
