...
public class Test2 implements Runnable {
    ...
    int n, tree[][], depth[][], size[], first[];

    void solve() throws IOException {
        int n = read.intNext(), q = read.intNext();

        depth = new int[2 * n - 1][2];
        size = iArr(n);
        first = iArr(n);
        ...
        for (int i = 0; i < n - 1; i++) {
            ctree.addEdge(read.intNext() - 1, i + 1, i);
        }
        ...
        SparseTable sparseTable = new SparseTable(depth);
        ...
        while (q-- > 0) {
            ...
            if (a == b) {
                ...
                sbr.append(sparseTable.rangeMinQuery(first[a], first[b]) + 1).append(' ');
    ...
    void dfs(int pos, int d, int parent) {
        int currDepth = d++;
        first[pos] = vertexPos;
        depth[vertexPos][0] = currDepth;
        depth[vertexPos++][1] = pos;
        size[pos] = 1;
    ...
    public static void main(String[] args) throws IOException {
        ...
        new Thread(null, new Test2(), "1", 1 << 26).start();
    }
