...
public class PuttingPlates implements Runnable {
         ...
         for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
               if (i == 0 || j == 0 || i == h - 1 || j == w - 1) {
                  boolean valid = true;
                  for (int[] d : dir) {
                     int modx = i + d[0], mody = j + d[1];
                     if (!isValid(modx, mody, h, w, ans)) {
                        valid = false;
                        break;
                     }
                  }
   ...
   boolean isValid(int i, int j, int h, int w, char[][] ans) {
      if (i >= 0 && j >= 0 && i < h && j < w && ans[i][j] == '1')
         return false;
      return true;
   }
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new PuttingPlates(), "1").start();
   }
