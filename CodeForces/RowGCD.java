...
public class RowGCD implements Runnable {
   ...
   void solve() throws IOException {
      ...
      long gcd = (arr.length > 1) ? Math.abs(arr[0] - arr[1]) : -1;
      for (int i = 2; i < n; i++) {
         gcd = find(gcd, Math.abs(arr[0] - arr[i]));
      }

      for (int i = 0; i < m; i++) {
         long num = rl();
         println((gcd != -1) ? find(arr[0] + num, gcd) : arr[0] + num);
      }
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new RowGCD(), "1").start();
   }
