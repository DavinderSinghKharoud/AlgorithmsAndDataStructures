...
public class PhoenixAndTowers implements Runnable {

   void solve() throws IOException {
      int t = ri();
      while (t-- > 0) {
         int n = ri(), m = ri(), x = ri();
         List<Tower> arr = new ArrayList<>();
         for (int i = 0; i < n; i++)
            arr.add(new Tower(ri(), i));

         arr.sort(Comparator.comparingInt(o -> o.val));
         PriorityQueue<long[]> pq = new PriorityQueue<>(Comparator.comparingLong(o -> o[1]));
         for (int i = 1; i <= m; i++) {
            arr.get(arr.size() - i).res = i;
            pq.add(new long[] { i, arr.get(arr.size() - i).val });
         }

         for (int i = arr.size() - m - 1; i >= 0; i--) {
            Tower curr = arr.get(i);
            long[] small = pq.poll();
            curr.res = small[0];
            small[1] += curr.val;
            pq.add(small);
         ...
         if (Math.abs(max - min) > x) {
            ...
            println("YES");
            long[] ans = lArr(n);
            arr.forEach(o -> ans[o.index] = o.res);
            Arrays.stream(ans).forEach(o -> print(o + " "));
            println("");
         }
      }
   }
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new PhoenixAndTowers(), "1").start();
   }
