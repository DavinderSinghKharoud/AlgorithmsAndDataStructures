...
public class SolutionToTheEquation implements Runnable {

    void solve() throws IOException {
        int t = ri();
        for (int tt = 1; tt <= t; tt++) {
            long a = rl(), b = rl(), c = rl(), minx = rl(), maxx = rl(), miny = rl(), maxy = rl();
            println(String.format("Case %s: %s", tt, findAllSol(a, b, -c, minx, maxx, miny, maxy)));
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        new Thread(null, new SolutionToTheEquation(), "1").start();
    }
