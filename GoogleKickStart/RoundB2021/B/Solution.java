package GoogleKickStart.RoundB2021.B;
...
public class Solution implements Runnable {
   ...
   int solve(int[] diff) {
      ...
      for (int i = 0, j = 0; i < len; i++) {
         ...
         if (j < len - 1 && diff[j] + diff[j + 1] == 2 * diff[i]) {
            int end = j + 2;
            while (end < len && diff[end] == diff[i]) {
               end++;
            }
            ans = max(ans, end - i + 1);
         }
   ...
   void reverse(int[] diff) {
      int s = 0, e = diff.length - 1;
      while (s <= e) {
         diff[s] = diff[s] ^ diff[e] ^ (diff[e] = diff[s]);
         s++;
         e--;
      }
   }
