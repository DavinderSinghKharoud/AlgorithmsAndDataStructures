...
public class MathEncoder implements Runnable {
   ...
   void solve() {
      ...
      for (int tt = 1; tt <= t; tt++) {
         ...
         long[] powers = lArr(len + 1);
         powers[0] = 1;
         powers[1] = 2;
         for (int i = 2; i <= len; i++) {
            powers[i] = mul(powers[i - 1], 2);
         }

         // We check for each number how many times it can set as bigger and smaller
         long res = 0;
         for (int i = 1; i <= len; i++) {
            int curr = ri();
            res = add(res, mul(curr, powers[i - 1]));
            res = sub(res, mul(powers[len - i], curr));
         }
         println("Case #" + tt + ": " + res);
      }
   }
   ...
   public static void main(String[] args) throws IOException {
      new Thread(null, new MathEncoder(), "1").start();
   }
