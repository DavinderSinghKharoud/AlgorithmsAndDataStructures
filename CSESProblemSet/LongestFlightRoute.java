...
public class LongestFlightRoute{
    ...
    static void solve() throws IOException {
		...
		int value = dfs( arr, count, child, 0);
    ...
    static int dfs( List<List<Integer>> arr, int[] count, int[] child, int node ){
		...
		for(int adj: arr.get(node)){
			int curr = dfs(arr, count, child, adj);
			if( max < curr ){
				max = curr;
				child[node] = adj;
			}
