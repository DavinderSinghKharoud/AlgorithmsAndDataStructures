...
public class TreeMatching {
    ...
    int mxN = (int) 2e5;
    int[][] dp; //dp[x][0]( node is not selected) and dp[x][1] ( node is selected)
    List<List<Integer>> tree;

    void solve() throws IOException {

        int n = read.intNext() - 1;
        dp = new int[n + 1][2];
    ...
    void dfs(int curr, int parent) {
        ...
        for (int child : tree.get(curr)) {
            if (child != parent) {
                dp[curr][1] = max(dp[curr][1], 1 + dp[child][0] + (dp[curr][0] - max(dp[child][0], dp[child][1])));

            }
            //We are subtracting the child to get the sum of other nodes
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        TreeMatching object = new TreeMatching();
        object.solve();
        out.close();
    }
