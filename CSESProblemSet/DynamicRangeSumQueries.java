...
public class DynamicRangeSumQueries {
    ...
    static void solve() throws IOException {
        ...
        while (m-- > 0) {
            int t = fastReader.intNext(), a = fastReader.intNext(), b = fastReader.intNext();
            ...
            if (t == 1) {
                ...
                println(findSum(dp, a, b));
            }
        }

    }

    static long findSum(long[] dp, int a, int b) {

        return find(dp, b) - find(dp, a - 1);
    }

    static long find(long[] dp, int pos) {
		long sum = 0;
		while( pos > 0 ){
			sum += dp[pos];
			pos -= ( pos & -pos );
    ...
    static void update(long[] dp, int pos, int value) {
        while (pos < dp.length) {
            dp[pos] += value;
            pos += (pos & -pos);
        }
